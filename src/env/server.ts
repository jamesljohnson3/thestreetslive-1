...
const facebookParser = makeValidator<string>((input) => {
  if (process.env.AUTH_PROVIDER === 'facebook' && input === '') {
    throw invalidEnvError('facebook config', input)
  }
  return input
})
...
export const serverEnv = {
  ...
  ...envsafe({
    ...
    AUTH_PROVIDER: str({
      choices: ['github', 'okta', 'google', 'facebook'],
    }),
    ...
    FACEBOOK_CLIENT_ID: facebookParser({ allowEmpty: true, default: '' }),
    FACEBOOK_CLIENT_SECRET: facebookParser({ allowEmpty: true, default: '' }),
